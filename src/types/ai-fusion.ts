 ...
 * AI 融合任务状态
 * - pending: 等待处理
 * - processing: DashScope 生成中
 * - saving: 正在保存结果（锁状态，防止重复创建）
 * - completed: 完成
 * - failed: 失败
 */
export type AIFusionTaskStatus = 'pending' | 'processing' | 'saving' | 'completed' | 'failed'
...
// 运行时类型守卫
export function isAIFusionTask(value: unknown): value is AIFusionTask {
